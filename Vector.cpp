...
// definition of vector operator []
// allows v.mData[i] to be written as v[i], as in python
double& Vector::operator[](int i)
{
...
// function to calculate the L^\infty norm
double max(const Vector& v)
{
	double big = abs(v.mData[0]);
	for(int i=1; i<v.mSize; i++)
	{
		if (abs(v.mData[i]) > big)
		{
			big = abs(v.mData[i]);
		}
	}
